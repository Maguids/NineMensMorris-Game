...
exports.notify = (req, res) => {
    ...
    if (currentGame.turn !== nick) {
        return res.status(403).json({ error: 'Não é a vez do jogador.' });
    }
    ...
    // Fase: move
        ...
        // Verificar vitória
        const new_counts = countPieces(currentGame.board, currentGame.players);
        const winner = checkWinner(currentGame, new_counts);
    ...
    if (currentGame.winner) {
        return res.status(200).json({ message: `Jogo terminou. Vencedor: ${currentGame.winner}` });
    }

    return res.status(200).json({ message: 'Jogada processada com sucesso.' });
};
