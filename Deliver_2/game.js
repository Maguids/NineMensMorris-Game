...
document.addEventListener('DOMContentLoaded', () => {
    ...
    playOnlineButton.addEventListener('click', () => {
        ...
        // Definir game-mode como pvs (player vs server)
        const radio = document.getElementById('player-vs-server');
        if (radio) {
            radio.checked = true; // Marca o botão de rádio com value="pvs"
        }
    });
...
function updateOfflineUI() {
    ...
    document.getElementById('logout-button').addEventListener('click', () => {
        ...
        location.reload(); // Refresh the page to restart the session
    });
}
...
async function leaveServer() {
    ...
    try {
        ...
        if (!response.ok) {
            const error = await response.json();
            throw new Error(error.error || "Erro ao desistir no servidor.");
        }
        ...
        console.error("Erro ao processar desistência:", error.message);
        alert(`Erro ao desistir: ${error.message}`);
    } finally {
...
function startGameUpdate(nick, gameId) {
    ...
    console.log("MOVMENT ESTÁ COMO: ", sessionStorage.getItem("movement"));
    const url = `http://twserver.alunos.dcc.fc.up.pt:8008/update?nick=${nick}&game=${gameId}`;
    ...
    eventSource.onerror = function(error) {
        ...
        setTimeout(() => {
            startGameUpdate(nick, gameId); // Reconnect with the same gameId
...
function handleGlobalUpdate(data, nick) {
    ...
    if (data.board) {
        ...
        if (isBoardEmpty) {
            ...
            // Set the first player based on turn information
            const select = document.getElementById('first-player');
            if (select) {
                select.value = data.turn === nick ? 'player1' : 'player2';
            }

            hideWaitingMessage(); // Hide the waiting message when the board is initialized
            restartGame(config); // Initialize the game
        } else {
            ...
            if (data.turn === nick) {
                ...
                if (data.phase === "move" && jogoAtual.fase !== 1 && data.step === "from" && m === "true") {
                    console.log(`O adversário moveu a peça.`);
                    const move = jogoAtual.encodeNode(data.cell);
                ...
                if (data.phase === "move" && data.step === "to") {
                    console.log(`O adversário selecionou uma peça.`);
                    const move = jogoAtual.encodeNode(data.cell);
                    jogoAtual.selecionarPecaParaMover(move);
                }
                
                if (data.phase === "move" && data.step === "from" && m === "true") {
                    console.log(`O adversário removeu a seleção da peça.`);
                    const move = jogoAtual.encodeNode(data.cell);
                    jogoAtual.removerSelecaoPecaParaMover(move);
                }

                if (data.phase === "move" && data.step === "take") {
                    console.log(`O adversário moveu a peça. Vai haver uma captura.`);
                    const move = jogoAtual.encodeNode(data.cell);
                    jogoAtual.moverPeca(move);
                ...
                if (data.phase === "move" && m === "false") {sessionStorage.setItem("movement", "true");}
            }
        }
    } else {
        showWaitingMessage(); // Show the waiting message if no board data is available
    }
...
document.getElementById("show-rankings").addEventListener("click", async () => {
    const group = 12; // Replace with your group's number
